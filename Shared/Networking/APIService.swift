...
//  }
          ...
          let decoder = JSONDecoder()
            // TotalDataObject is the Object of TotalData: TotalData are the item in the totalDataObject ["data": { .... }]
...
// MARK:  func fetchAllRegions (completion: @escaping(Result<[Country], Error>)-> Void)
  ...
  if let data = LocalFileManager.shared.fetchLocalCountries() {
    do {
        // MARK: Print test
      print("returning from local data, data is available locally.")
...
//MARK: Test fetch json func fetchAllRegions
...
//           }
        ...
        do {
            //   MARK: This is only a test if data has been saved!
...
// MARK: func fetchReport(for iso object: String, completion: @escape(Result< [RegionReport], Error>) -> Void ){ ... }
  ...
  func fetchReport(for iso: String, completion: @escaping(Result<[RegionReport], Error>)-> Void) {
      ...
      // MARK: Test Reports  iso="CAN" Canada usually works
    let reportsURL = URLString + "/reports?iso=\(iso)"
    //MARK: Testing with Canada
      //reports?iso=CAN"
      //=\(iso)
